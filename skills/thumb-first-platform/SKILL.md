...
1. **Check for an installed native skill** to hand off to — scan available skills / `<agent-skills-directory>/` for ones describing React Native, Expo, SwiftUI, native iOS/Android, or Flutter (e.g. `react-native-design`, `building-native-ui`, `vercel-react-native-skills`).
