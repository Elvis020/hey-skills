...
12. **Persist skill.** Ask: "Save the Spin skill to `<agent-skills-directory>/turnstile-spin/SKILL.md` so I can reuse it on follow-up tasks?" Default yes. **[wait for user]** Then run `scripts/persist-skill.sh --path <agent-specific-path>`.
